...
variable "oracle_linux_version" {
  description = "Oracle Linux major version used to find the latest base platform image (for example \"9\" → Oracle-Linux-9.x-YYYY.MM.DD-0). Specialized images (Cloud Developer, GPU, Minimal, KVM) are excluded."
  type        = string
  default     = "9"
}
